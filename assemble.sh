...
CONFIG_PATH="/media/alejandra/WildType/proyecto_xoca_2020/config.txt"
CMD_PATH="/home/alejandra/MaSuRCA-3.4.2/bin/masurca"
set -o pipefail
...
#}
export PATH="/home/alejandra/MaSuRCA-3.4.2/bin/../CA8/Linux-amd64/bin:/home/alejandra/MaSuRCA-3.4.2/bin:$PATH"
save PATH
export PERL5LIB=/home/alejandra/MaSuRCA-3.4.2/bin/../lib/perl${PERL5LIB:+:$PERL5LIB}
save PERL5LIB
NUM_THREADS=20
save NUM_THREADS
log 'Processing pe library reads'
rm -rf meanAndStdevByPrefix.pe.txt
echo 'ag 195 63' >> meanAndStdevByPrefix.pe.txt
rename_filter_fastq 'ag' <(exec expand_fastq '/media/alejandra/WildType/proyecto_xoca_2020/A5_fil_R1.fastq' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}') <(exec expand_fastq '/media/alejandra/WildType/proyecto_xoca_2020/A5_fil_R2.fastq' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}' ) > 'ag.renamed.fastq'
echo 'gs 96 26' >> meanAndStdevByPrefix.pe.txt
rename_filter_fastq 'gs' <(exec expand_fastq '/media/alejandra/WildType/proyecto_xoca_2020/Ags_fil_R1.fastq' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}') <(exec expand_fastq '/media/alejandra/WildType/proyecto_xoca_2020/Ags_fil_R2.fastq' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}' ) > 'gs.renamed.fastq'
echo 'mg 443 82' >> meanAndStdevByPrefix.pe.txt
rename_filter_fastq 'mg' <(exec expand_fastq '/media/alejandra/WildType/proyecto_xoca_2020/Ag5_fil_R1.fastq' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}') <(exec expand_fastq '/media/alejandra/WildType/proyecto_xoca_2020/Ag5_fil_R2.fastq' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}' ) > 'mg.renamed.fastq'

head -q -n 40000  ag.renamed.fastq gs.renamed.fastq mg.renamed.fastq | grep --text -v '^+' | grep --text -v '^@' > pe_data.tmp
...
KMER=`for f in ag.renamed.fastq gs.renamed.fastq mg.renamed.fastq;do head -n 80000 $f |tail -n 40000;done | perl -e 'while($line=<STDIN>){$line=<STDIN>;chomp($line);push(@lines,$line);for($i=0;$i<6;$i++){$line=<STDIN>;}}$min_len=100000;$base_count=0;foreach $l(@lines){$base_count+=length($l);push(@lengths,length($l));@f=split("",$l);foreach $base(@f){if(uc($base) eq "G" || uc($base) eq "C"){$gc_count++}}} @lengths =sort {$b <=> $a} @lengths; $min_len=$lengths[int($#lengths*.75)];  $gc_ratio=$gc_count/$base_count;$kmer=0;if($gc_ratio>=0.35 && $gc_ratio<=0.6){$kmer=int($min_len*.66);}else{$kmer=int($min_len*.33);} $kmer++ if($kmer%2==0); $kmer=31 if($kmer<31); $kmer=127 if($kmer>127); print $kmer'`
save KMER
log Using kmer size of $KMER for the graph
KMER_J=$KMER
MIN_Q_CHAR=`cat ag.renamed.fastq gs.renamed.fastq mg.renamed.fastq |head -n 50000 | awk 'BEGIN{flag=0}{if($0 ~ /^\+/){flag=1}else if(flag==1){print $0;flag=0}}'  | perl -ne 'BEGIN{$q0_char="@";}{chomp;@f=split "";foreach $v(@f){if(ord($v)<ord($q0_char)){$q0_char=$v;}}}END{$ans=ord($q0_char);if($ans<64){print "33\n"}else{print "64\n"}}'`
save MIN_Q_CHAR
log MIN_Q_CHAR: $MIN_Q_CHAR
JF_SIZE=`ls -l *.fastq | awk '{n+=$5}END{s=int(n/50); if(s>1700000000)printf "%.0f",s;else print "1700000000";}'`
save JF_SIZE
perl -e '{if(int('$JF_SIZE')>1700000000){print "WARNING: JF_SIZE set too low, increasing JF_SIZE to at least '$JF_SIZE', this automatic increase may be not enough!\n"}}'
log Creating mer database for Quorum
awk '{print substr($0,1,200)}' ag.renamed.fastq gs.renamed.fastq mg.renamed.fastq | quorum_create_database -t 20 -s $JF_SIZE -b 7 -m 24 -q $((MIN_Q_CHAR + 5)) -o quorum_mer_db.jf.tmp /dev/stdin && mv quorum_mer_db.jf.tmp quorum_mer_db.jf
if [ 0 != 0 ]; then
...
quorum_error_correct_reads  -q $(($MIN_Q_CHAR + 40)) --contaminant=/home/alejandra/MaSuRCA-3.4.2/bin/../share/adapter.jf -m 1 -s 1 -g 1 -a 3 -t 20 -w 10 -e 3 -M  quorum_mer_db.jf ag.renamed.fastq gs.renamed.fastq mg.renamed.fastq --no-discard -o pe.cor.tmp --verbose 1>quorum.err 2>&1 && mv pe.cor.tmp.fa pe.cor.fa || fail Error correction of PE reads failed. Check pe.cor.log.
...
jellyfish count -m 31 -t 20 -C -s $JF_SIZE -o k_u_hash_0 pe.cor.fa
export ESTIMATED_GENOME_SIZE=`jellyfish histo -t 20 -h 1 k_u_hash_0 | tail -n 1 |awk '{print $2}'`
...
create_k_unitigs_large_k -c $(($KMER-1)) -t 20 -m $KMER -n $(($ESTIMATED_GENOME_SIZE*2)) -l $KMER -f `perl -e 'print 1/'$KMER'/1e5'` pe.cor.fa  | grep --text -v '^>' | perl -ane '{$seq=$F[0]; $F[0]=~tr/ACTGactg/TGACtgac/;$revseq=reverse($F[0]); $h{($seq ge $revseq)?$seq:$revseq}=1;}END{$n=0;foreach $k(keys %h){print ">",$n++," length:",length($k),"\n$k\n"}}' > guillaumeKUnitigsAtLeast32bases_all.fasta.tmp && mv guillaumeKUnitigsAtLeast32bases_all.fasta.tmp guillaumeKUnitigsAtLeast32bases_all.fasta
...
create_k_unitigs_large_k -c $(($KMER_J-1)) -t 20 -m $KMER_J -n $(($ESTIMATED_GENOME_SIZE*2)) -l $KMER_J -f `perl -e 'print 1/'$KMER_J'/1e5'` pe.cor.fa  | grep --text -v '^>' | perl -ane '{$seq=$F[0]; $F[0]=~tr/ACTGactg/TGACtgac/;$revseq=reverse($F[0]); $h{($seq ge $revseq)?$seq:$revseq}=1;}END{$n=0;foreach $k(keys %h){print ">",$n++," length:",length($k),"\n$k\n"}}' > guillaumeKUnitigsAtLeast32bases_all.jump.fasta.tmp && mv guillaumeKUnitigsAtLeast32bases_all.jump.fasta.tmp guillaumeKUnitigsAtLeast32bases_all.jump.fasta 
fi
...
rm -rf work1
CA_DIR="CA";
createSuperReadsForDirectory.perl -l $KMER -mean-and-stdev-by-prefix-file meanAndStdevByPrefix.pe.txt -kunitigsfile guillaumeKUnitigsAtLeast32bases_all.fasta -t 20 -mikedebug work1 pe.cor.fa 1> super1.err 2>&1
...
grep --text -A 1 '^>ag' pe.linking.fa | grep --text -v '^\-\-' | sample_mate_pairs.pl $MAX_LINKING_MATES $NUM_LINKING_MATES 1 > ag.tmp && error_corrected2frg ag 195 63 2000000000 ag.tmp > ag.linking.frg.tmp && rm ag.tmp && mv ag.linking.frg.tmp ag.linking.frg
grep --text -A 1 '^>gs' pe.linking.fa | grep --text -v '^\-\-' | sample_mate_pairs.pl $MAX_LINKING_MATES $NUM_LINKING_MATES 1 > gs.tmp && error_corrected2frg gs 96 26 2000000000 gs.tmp > gs.linking.frg.tmp && rm gs.tmp && mv gs.linking.frg.tmp gs.linking.frg
grep --text -A 1 '^>mg' pe.linking.fa | grep --text -v '^\-\-' | sample_mate_pairs.pl $MAX_LINKING_MATES $NUM_LINKING_MATES 1 > mg.tmp && error_corrected2frg mg 443 82 2000000000 mg.tmp > mg.linking.frg.tmp && rm mg.tmp && mv mg.linking.frg.tmp mg.linking.frg
...
runCA -s runCA.spec stopAfter=initialStoreBuilding -p genome -d CA cgwErrorRate=0.15 doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg   ag.linking.frg gs.linking.frg mg.linking.frg    1> runCA0.out 2>&1
...
runCA -s runCA.spec stopBefore=scaffolder -p genome -d CA cgwErrorRate=0.15 doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg   ag.linking.frg gs.linking.frg mg.linking.frg    1> runCA1.out 2>&1

if [ ! -d CA/1-overlapper ]; then
  rm -f CA/0-overlaptrim-overlap/overlap.sh CA/1-overlapper/overlap.sh && 
  runCA -s runCA.spec -p genome -d CA stopBefore=scaffolder doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg   ag.linking.frg gs.linking.frg mg.linking.frg    1>>runCA1.out 2>&1
fi
...
jellyfish count -L 2 -C -m 30 -s $ESTIMATED_GENOME_SIZE -t 20 -o unitig_mers /dev/fd/0 && 
cat <(overlapStore -b 1 -e $NUM_SUPER_READS -d genome.ovlStore  | awk '{if($1<$2 && ($1<'$NUM_SUPER_READS' && $2<'$NUM_SUPER_READS')) print $0}'|filter_overlap_file -t 20 <(gatekeeper  -dumpfragments -withsequence genome.gkpStore| grep -P '^fragmentIdent|^fragmentSequence' | awk 'BEGIN{flag=1}{if(flag){print ">"$3}else{ print $3;} flag=1-flag; }') unitig_mers /dev/fd/0) <(overlapStore -d genome.ovlStore | awk '{if($1<$2 && ($1>='$NUM_SUPER_READS' || $2>='$NUM_SUPER_READS')) print $1" "$2" "$3" "$4" "$5" "$6" "$7}')  |convertOverlap -ovl |gzip > overlaps_dedup.ovb.gz &&  
...
runCA -s runCA.spec stopBefore=scaffolder -p genome -d CA cgwErrorRate=0.15 doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg   ag.linking.frg gs.linking.frg mg.linking.frg    1> runCA2.out 2>&1 
log "Recomputing A-stat for super-reads"
recompute_astat_superreads_CA8.sh genome CA $PE_AVG_READ_LENGTH work1/readPlacementsInSuperReads.final.read.superRead.offset.ori.txt superReadSequences_shr.frg
fi
...
if [ -s $CA_DIR/9-terminator/genome.scf.fasta ];then
  ...
  if [ $NCTG -eq $NSCF ];then
    ...
    if [ -s $CA_DIR/10-gapclose/genome.scf.fasta ];then
      ...
      log 'Gap closing'
      closeGapsLocally.perl --max-reads-in-memory 1000000000 -s 1700000000 --Celera-terminator-directory $CA_DIR/9-terminator --reads-file 'ag.renamed.fastq' --reads-file 'gs.renamed.fastq' --reads-file 'mg.renamed.fastq' --output-directory $CA_DIR/10-gapclose --min-kmer-len 17 --max-kmer-len $(($PE_AVG_READ_LENGTH-5)) --num-threads 20 --contig-length-for-joining $(($PE_AVG_READ_LENGTH-1)) --contig-length-for-fishing 200 --reduce-read-set-kmer-size 21 1>gapClose.err 2>&1
...
if [ -s $CA_DIR/$TERMINATOR/genome.scf.fasta ];then
  ...
  log 'Removing redundant scaffolds'
    ...
    deduplicate_contigs.sh $CA_DIR genome 20 $PLOIDY $TERMINATOR && log "Assembly complete, final scaffold sequences are in $CA_DIR/final.genome.scf.fasta"
  else
