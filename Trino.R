...
if (length(args)==0) {
  file <- "Resultados_Total_Trembl_TAIR.xls"
} else 
  file = args[1]
...
#png("DistribucionDatos.png", 1000, 800, pointsize=20)
print(p)
#dev.off()
...
go <- split_GO(x,"gene_ontology_BLASTX")
gos <- summary_GO(go)
...
#png("DistribucionTrans.png", 1000, 800, pointsize=20)
plot_NOGs(x, "transcript_id")

#dev.off()

#png("DistribucionTGene.png", 1000, 800, pointsize=20)
plot_NOGs(x, "gene_id")
#dev.off()

#write.csv(x, file = "Nogs.csv")
...
#png("DistribucionTGO.png", 1000, 800, pointsize=20)
...
names(xgenus) <- c("Géneros", "Números")
xgenus <- xgenus[order(-xgenus$Números), ]
xgenus[(Números <= 200), Géneros := "Otros"]
#length(table(xgenus$Genus))

xgenus$Tipo <-"Transcritos"
genus$Tipo <-"ORF"

plot <- rbind(xgenus)
#plot <- rbind(xgenus,genus)

#png("Generos.png", 1000, 800, pointsize=20)

ggbarplot(plot, x = "Géneros", y = "Números",
          fill = "Tipo",               # change fill color by cyl
          color = "Tipo",            # Set bar border colors to white
...
write.table(blastx, file= "blastx.tsv", sep="\t", row.names = F, col.names = T)
write.table(go, file=paste0(outpath, "/", file, ".go.tsv"), sep="\t", row.names = F, col.names = T)
write.table(go,"go.tsv", sep="\t", row.names = F, col.names = T)
