...
mls <- read.csv("res05UD-T72.csv")
x <- x[x$transcript_id %in% mls$genes,]
...
png("DistribucionDatosT72.png", 1000, 1000, pointsize=20)
print(p)
dev.off()
...
#write.csv(egg,"eggNog_2.csv")

png("DistribucionTransT72.png", 1000, 1000, pointsize=20)
plot_NOGs(x, "transcript_id")
dev.off()

png("DistribucionTGeneT72.png", 1000, 1000, pointsize=20)
plot_NOGs(x, "gene_id")
dev.off()


write.csv(x, file = "NogsT72.csv")

#system("rm NOG.annotations.tsv")

# ====
library(ggpubr)
plotgos <- head(gos[order(-gos$transcripts),], 60)
write.csv(gos, file = "GOT72.csv")
...
bar <- ggbarplot(plotgos, "name", "transcripts",
                 ...
                 sort.by.groups = TRUE,
                 title = "Highest ontologies (top 60)")
...
png("DistribucionTGeneT72.png", 1000, 1000, pointsize=20)
bar + theme(axis.text.x = element_text(angle = 75, hjust = 1, size = 8))
dev.off()
#dev.off()
...
plot[,plot$Genus=="others"]
plot2 <- head(plot[order(-plot$Number),],50)

png("GenerosT72.png", 1000, 1000, pointsize=20)

ggbarplot(plot2, x = "Genus", y = "Number",
...
# ================
# Save image first
save.image(file = paste0(file, "T72.RData"))

# ======================
# Save annotation outputs
# ======================

write.table(blastx, file=paste0(outpath, "/", file, ".blastxT72.tsv"), sep="\t", row.names = F, col.names = T)
write.table(blastx, file= "blastxT72.tsv", sep="\t", row.names = F, col.names = T)
write.table(go, file=paste0(outpath, "/", file, ".goT72.tsv"), sep="\t", row.names = F, col.names = T)
write.table(go,"goT72.tsv", sep="\t", row.names = F, col.names = T)
...
htmlwidgets::saveWidget(widget, paste0(file, "T72.pfam.html"))
